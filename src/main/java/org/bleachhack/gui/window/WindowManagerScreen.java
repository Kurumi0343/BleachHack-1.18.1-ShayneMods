...
public class WindowManagerScreen extends Screen {
	...
	public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
		...
		DrawableHelper.fill(matrices, 0, height - 13, 19, height - 12, 0xf44336);
		DrawableHelper.fill(matrices, 0, height - 13, 1, height, 0xf44336);
		DrawableHelper.fill(matrices, 19, height - 12, 20, height, 0xf44336);
		textRenderer.draw(matrices, "\u00a7cX", 7, height - 10, -1);

		int wid = 20;
		int size = Math.min((width - wid) / windows.length, 90);
		for (int i = 0; i < windows.length; i++) {
			DrawableHelper.fill(matrices, wid, height - 13, wid + size - 1, height - 12, 0xf44336);
			DrawableHelper.fill(matrices, wid, height - 13, wid + 1, height, 0xff6060b0);
			DrawableHelper.fill(matrices, wid + size - 1, height - 12, wid + size, height, 0xf44336);
