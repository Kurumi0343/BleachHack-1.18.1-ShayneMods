...
public class Window {
	...
	protected void drawBackground(MatrixStack matrices, int mouseX, int mouseY, TextRenderer textRend) {
		/* background */
		DrawableHelper.fill(matrices, x1, y1 + 1, x1 + 1, y2 - 1, 0xcc0000);
		horizontalGradient(matrices, x1 + 1, y1, x2 - 1, y1 + 1, 0xcc0000, 0xfffccff);
		DrawableHelper.fill(matrices, x2 - 1, y1 + 1, x2, y2 - 1, 0xffccff);
		horizontalGradient(matrices, x1 + 1, y2 - 1, x2 - 1, y2, 0xffccff, 0xffccff);

		DrawableHelper.fill(matrices, x1 + 1, y1 + 12, x2 - 1, y2 - 1, 0xffccff);

		/* title bar */
		horizontalGradient(matrices, x1 + 1, y1 + 1, x2 - 1, y1 + 12, (selected ? 0xffccff : 0xcc0000), (selected ? 0xffccff : 0xffa0a0a0));
	...
	public static void fill(MatrixStack matrices, int x1, int y1, int x2, int y2) {
		fill(matrices, x1, y1, x2, y2, 0xcc0000, 0xffccff, 0x00000000);
	}

	public static void fill(MatrixStack matrices, int x1, int y1, int x2, int y2, int fill) {
		fill(matrices, x1, y1, x2, y2, 0xcc0000, 0xffccff, fill);
	}
