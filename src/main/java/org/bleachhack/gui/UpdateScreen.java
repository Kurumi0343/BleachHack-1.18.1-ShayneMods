...
public class UpdateScreen extends WindowScreen {
	...
	private final Set<WindowWidget> changelogWidgets = new HashSet<>();
	...
	public void init() {
		...
		addWindow(new Window(width / 2 - wd,
				height / 16,
				width / 2 + wd,
				height - height / 16, String.format("ShayneMods Update [%s -> %s]", BleachHack.VERSION, updateJson.get("name").getAsString()), new ItemStack(Items.MAGENTA_GLAZED_TERRACOTTA)));

		int w = getWindow(0).x2 - getWindow(0).x1;
		int h = getWindow(0).y2 - getWindow(0).y1;

		getWindow(0).addWidget(new WindowTextWidget("A new ShayneMods update is available.", true, WindowTextWidget.TextAlign.MIDDLE, 1.5f, w / 2, 18, 0xe0e0e0));
