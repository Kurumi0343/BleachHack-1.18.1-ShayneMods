...
public class ModuleClickGuiScreen extends ClickGuiScreen {
	...
	public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
		...
		textRenderer.draw(matrices, "ShayneMods-" + BleachHack.VERSION + "-" + SharedConstants.getGameVersion().getName(), 3, 3, 0xcc0000);
		...
		if (clickGui.getSetting(2).asToggle().getState()) {
			...
			textRenderer.drawWithShadow(matrices, "Use " + Command.getPrefix() + "clickgui to reset the clickgui", 2, height - 10, 0xe06666);
		}
	}
}
