...
public class BleachTitleScreen extends WindowScreen {
	...
	public void init() {
		...
		addWindow(new Window(width / 8,
				height / 8,
				width - width / 8,
				height - height / 8, "ShayneMods", new ItemStack(Items.DRAGON_HEAD)));
		...
		getWindow(0).addWidget(new WindowTextWidget(LiteralText.EMPTY, true, WindowTextWidget.TextAlign.MIDDLE, 3f, w / 2, h / 4 - 25, 0)
				.withRenderEvent((widget, ms, wx, wy) -> {
					...
					int i = 0;
					for (char c: "ShayneMods".toCharArray()) {
						int fi = i++;
						bhText.append(
		...
		// Version Text
		getWindow(0).addWidget(new WindowTextWidget(BleachHack.VERSION, true, WindowTextWidget.TextAlign.MIDDLE, 1.5f, w / 2, h / 4 - 6, 0xc90076));

		// Splash
		getWindow(0).addWidget(new WindowTextWidget(LiteralText.EMPTY, true, WindowTextWidget.TextAlign.MIDDLE, 2f, -20f, w / 2 + 80, h / 4 + 6, 0xf44336)
				.withRenderEvent((widget, ms, wx, wy) -> {
					if (splash != null) {
